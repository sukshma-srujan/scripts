...
(function() {
  ...
  function findAnchors() {
    const lgs = document.querySelectorAll('legend > b');
    const neighbourClassName = 'jk-image-container-neighbour';
    for (let lg of lgs) {
      if (lg.innerText.toLowerCase() === 'description') {
        lg.parentNode.classList.add(neighbourClassName);
        const ic = document.querySelector(`.${neighbourClassName} ~ .container-fluid`);
        if (ic) {
          ic.classList.add('jk-image-container');
          return ic.querySelectorAll("a");
        }
      }
    }
    return [];
