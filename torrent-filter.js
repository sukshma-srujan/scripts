...
ui.init = function () {
    ...
    ui.tableData = [];
    const tableContents = document.querySelectorAll(dataSpec.rowSel);

    for (let htmlData of tableContents) {
        const rowData = new RowData(htmlData, dataSpec);
        ui.tableData.push(rowData);
    }
...
ui.applyFilters = function () {
    ...
    for (let rowData of ui.tableData) {
    ...
    log.info("Filters applied successfully. Showing " + rowCount + " of " + ui.tableData.length);
}
